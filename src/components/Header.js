  ...
  render() {
    return (
      <ImageBackground source={HeaderBgImg}  style={styles.headerStyle}>
        ...
        <View style={{ flex: 1, padding: 15, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between'}}>

        <View style={{  flexDirection: 'row', justifyContent: 'space-between', width: width/1.8}}>
         ...
         {
           this.props.showSetting ? <TouchableOpacity onPress={() => this.props.navigateToSettings()}>
                 <Image source={SettingIcon} style={{ height: 40 , width: 40 }} />
            </TouchableOpacity>
          : null
         }

          <Text numberOfLines={1} style={[styles.headerTextStyle, this.props.headingStyle]}>{this.props.headerText}</Text>
          ...
          {
            this.props.showSideMenu ? <View style={{ flexDirection: 'row', alignItems: 'center'}}>
            <TouchableOpacity onPress={() => this.props.ShowAll()} style={{ flexDirection: 'row', alignItems: 'center'}}>
               <Text style={{ color : "#fff", marginRight:  9, fontWeight: 'bold',}}>Tout</Text>
            </TouchableOpacity>
            <Menu
            ref={this.setMenuRef}
            style={{ backgroundColor: "#3362CA", marginTop:  17}}
            button={<TouchableOpacity onPress={() => this.showMenu()}>
               <Image source={SideMenuImg} style={{ height: 14 , width: 14 }} />
               </TouchableOpacity>
             }
            >
