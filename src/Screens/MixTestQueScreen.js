...
export default class MixTestQueScreen extends Component {
  constructor(props) {
  ...
  nextQue() {
    ...
    if(this.state.current_que < questionlimit - 1 ){
 ...
 closePress(){
  ...
  let topic = {
   ...
   const TopicIDLocal = topicProgress.filter((item) => item.topic_id == 0)

   console.log("TopicIDLocal", TopicIDLocal)

   if(TopicIDLocal.topic_id  == "0"){
     realm.write(() => {
       topicProgress.question_attempt = this.state.current_que + 1
    })
   } else{
     realm.write(() => {
        realm.create('topic_progress', {
          topic_id: "0",
          question_attempt:  this.state.current_que + 1
          })
      })
   }
