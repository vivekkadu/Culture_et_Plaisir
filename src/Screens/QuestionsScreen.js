...
export default class QuestionsScreen extends Component {
  ...
  componentDidMount() {
    ...
    var topicProgress = realm.objects("topic_progress");
    var topicProgressData = topicProgress.filter(
      item => item.topic_id == topic.id
    );

    var progress = topicProgressData.map(item => item.question_attempt);

    this.setState({ lastQueIndex: questionlimit - progress });
  ...
  async option1Selected(scoreCut) {
    ...
    const questions = navigation.getParam("questions");
    const questionlimit = navigation.getParam("questionlimit");
    const lastQueIndex = questionlimit - this.state.current_que;

    this.setState({
      optionBackground1: "#000",
      optionBackground2: "#052666",
      optionBackground3: "#052666"
    });
    ...
    this.setState({
      attended_questions: [
        ...this.state.attended_questions,
        questions[this.state.current_que]
      ],
      selectedAns: 1,
      optionBackground1: "#000",
      optionBackground2: "#052666",
        ...
        this.setState(
          {
            ...
            correct_answers: [
              ...this.state.correct_answers,
              questionsNav[this.state.current_que]
            ]
          },
          () => {
            if (lastQueIndex == 1) {
  ...
  previosQue() {
    this.setState({
      proviousOption: this.state.proviousOption - 1,
      current_que: this.state.current_que - 1,
      optionBackground1: "#052666",
      optionBackground2: "#052666",
      optionBackground3: "#052666"
    });
    ...
    var filterData = answers.filter(
      item => item.queId == this.state.current_que - 1
    );
  ...
  closePress() {
    ...
    const TopicIDLocal = topicProgress.filter(
      item => item.topic_id == topic.id
    );
    ...
    this.props.navigation.navigate("ResultScreen", {
      answers: answers,
      topic: topic,
      attended_questions: this.state.attended_questions,
      ...
      <ImageBackground source={HomeBg} style={styles.container}>
        <View
          style={{
            ...
            marginTop: 15,
            padding: 10,
            backgroundColor: "#052666",
            borderRadius: 5
          }}
        >
          <Text style={{ color: "#fff", fontSize: 19 }}>
            Q {this.state.current_que + 1}.{" "}
            {questions[this.state.current_que].question}
          </Text>
        </View>

        <TouchableOpacity
          disabled={this.state.disableButton}
          onPress={() => this.option1Selected()}
          style={[
            { margin: 5, padding: 10, borderRadius: 5 },
            { backgroundColor: this.state.optionBackground1 }
          ]}
        >
          <Text style={{ color: "#fff", fontSize: 19 }}>
            {questions[this.state.current_que].answer_1}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          disabled={this.state.disableButton}
          onPress={() => this.option2Selected()}
          style={[
            { margin: 5, padding: 10, borderRadius: 5 },
            { backgroundColor: this.state.optionBackground2 }
          ]}
        >
          <Text style={{ color: "#fff", fontSize: 19 }}>
            {questions[this.state.current_que].answer_2}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          disabled={this.state.disableButton}
          onPress={() => this.option3Selected()}
          style={[
            { margin: 5, padding: 10, borderRadius: 5 },
            ,
            { backgroundColor: this.state.optionBackground3 }
          ]}
        >
          <Text style={{ color: "#fff", fontSize: 19 }}>
            {questions[this.state.current_que].answer_3}
          </Text>
        </TouchableOpacity>
        ...
        <View style={styles.bottomView}>
          ...
          >
            ...
            <TouchableOpacity
              onPress={() =>
                this.highlightAns(
                  questions[this.state.current_que].correct_answer
                )
              }
            >
            ...
            {this.state.current_que < questionlimit - 1 ? (
              <TouchableOpacity onPress={() => this.nextQue()}>
